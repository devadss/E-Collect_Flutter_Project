...
class TransactionReportRepository {
  ...
  Future<TransactionReport> getTransactionReportByMerchantIdDate(
      String merchantID, String fromDate , String toDate) async {
    final uri = Uri.parse(
        "https://dev.collect.org.in/api/Payment/transaction-history?merchantId=$merchantID&fromDate=$fromDate&toDate=$toDate");
    final request =
    await http.get(uri, headers: {"Content-Type": "application/json"});
    print("TransactionReport ${request.body}");
    if (request.statusCode == 200) {
