...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin {
  ...
  String _currentFilterPeriod = 'THIS_MONTH'; // Track current filter period
  ...
  //
  //             bool isApplyEnabled =
  //                 selectedIndex == 4 && fromDate != null && toDate != null;
  //
  //             return Padding(
  //               padding: const EdgeInsets.all(16),
  //               child: Column(
  ...
  //                           await qrProvider.getQrTranscationHistory(period,
  //                               formatted, formatted, 'COLLECTION', corpCode);
  ...
  //                             _isFilterApplied = true;
  //                             _currentFilterPeriod = period;
  //                             _currentFromDate = formatted;
  //                             _currentToDate = formatted;
  //                           });
  //                           break;
  //                         case 1:
  //                           period = 'THIS_WEEK';
  ...
  //
  //                           final formattedFdate =
  //                               DateFormat('yyyy-MM-dd').format(today);
  //                           final formattedTdate =
  ...
  //                               formattedFdate, formattedTdate, subAgentID!);
  //                           setState(() {
  //                             _isFilterApplied = true;
  //                             _currentFilterPeriod = period;
  //                             _currentFromDate = formattedFdate;
  //                             _currentToDate = formattedTdate;
  //                           });
  //
  //                           break;
  //                         case 2:
  ...
  //                               now.year, now.month, now.day); // midnight today
  //
  //                           final fromDate =
  ...
  //
  //                           final formattedFdate = DateFormat('yyyy-MM-dd')
  //                               .format(firstDayLastMonth);
  //                           final formattedTdate = DateFormat('yyyy-MM-dd')
  ...
  //                             _isFilterApplied = true;
  //                             _currentFilterPeriod = period;
  //                             _currentFromDate = "$firstDayLastMonth";
  //                             _currentToDate = "$lastDayLastMonth";
  //                           });
  //
  //                           break;
  //                         case 4:
  ...
  Future<void> _clearFilters() async {
    ...
    setState(() {
      ...
      _currentFilterPeriod = 'THIS_MONTH';
      _currentFromDate = '';
      _currentToDate = '';
    });
  }
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          switch (selectedIndex) {
                            ...
                            case 1: // THIS_WEEK
                              ...
                              DateTime endOfWeek = startOfWeek
                                  .add(const Duration(days: 6)); // Sunday
                              from =
                          ...
                          await Future.wait([
                            qrProvider.getQrTranscationHistory(
                                period, from, to, 'COLLECTION', corpCode),
                            cashTransProvider.getCashTranscationHistory(
                                ...
                                subAgentID,
                                corpCode),
                            linkProvider.getLinkTransactionHistory(
                                period, from, to, subAgentID!),
                          ]);

                          // Calculate sums in one go (no multiple setState)
                          setState(() {
                            qrAmt = qrProvider.qrTranscationHistoryModel?.data
                                    ?.fold(
                                        0,
                                        (sum, item) =>
                            ...
                            cashAmt = cashTransProvider
                                    .qrTranscationHistoryModel?.data
                                    ?.fold(
                                        0,
                                        (sum, item) =>
                                            sum! + (item.orderAmount ?? 0)) ??
                      ...
                      onPressed: () async {
                        ...
                        switch (selectedIndex) {
                          case 0:
                            ...
                            if(qrProvider.qrTranscationHistoryModel != null && qrProvider.qrTranscationHistoryModel?.data?.isNotEmpty == true){
                              int qrAmtLen = 0;
                              qrAmt = 0;
                              while (qrAmtLen < qrProvider.qrTranscationHistoryModel!.data!.length){
                                setState(() {
                                  qrAmt += qrProvider.qrTranscationHistoryModel!.data![qrAmtLen].orderAmount!;
                                });
                                qrAmtLen++;
                              }
                            }else{
                            ...
                            if(cashTransProvider.qrTranscationHistoryModel != null&& cashTransProvider.qrTranscationHistoryModel?.data?.isNotEmpty == true){
                              int cashAmtLen = 0;
                              cashAmt = 0;
                              while (cashAmtLen < cashTransProvider.qrTranscationHistoryModel!.data!.length){
                                setState(() {
                                  cashAmt += cashTransProvider.qrTranscationHistoryModel!.data![cashAmtLen].orderAmount!;
                                });
                                cashAmtLen++;
                              }
                            }else{
                          ...
                          case 1:
                            ...
                            if(cashTransProvider.qrTranscationHistoryModel != null){
                              int cashAmtLen = 0;
                              cashAmt = 0;
                              while (cashAmtLen < cashTransProvider.qrTranscationHistoryModel!.data!.length){
                                setState(() {
  ...
  Future<void> loadSharedPrefs(BuildContext context) async {
    ...
    if (mounted) {
      setState(() {
        ...
        subAgentID = subAgID;
        corpCode = crpCode;
        qrAmt = 0; // Reset before recalculating
        cashAmt = 0; // Reset before recalculating
      });
    }
    ...
    if (provider.qrTranscationHistoryModel != null) {
      setState(() {
        qrAmt = provider.qrTranscationHistoryModel!.data!
            .fold(0, (sum, item) => sum + (item.orderAmount ?? 0));
      });
    }
    ...
    if (cashTransProvider.qrTranscationHistoryModel != null) {
      setState(() {
        cashAmt = cashTransProvider.qrTranscationHistoryModel!.data!
            .fold(0, (sum, item) => sum + (item.orderAmount ?? 0));
      });
    }
...
/*  Future<void> loadSharedPrefs(BuildContext context) async {
    ...
    await provider.getQrTranscationHistory("THIS_MONTH", formattedFdate, formattedTdate, "COLLECTION", corpCode);

    if(provider.qrTranscationHistoryModel != null){
      int qrAmtLen = 0;
      while (qrAmtLen < provider.qrTranscationHistoryModel!.data!.length){
        setState(() {
          qrAmt += provider.qrTranscationHistoryModel!.data![qrAmtLen].orderAmount!;
        });
        qrAmtLen++;
      }
    }
    ...
    final linkProvider = Provider.of<LinkTransactionHistoryProvider>(context, listen: false,);
    final cashTransProvider = Provider.of<CashTransactionHistoryProvider>(context, listen: false,);

    await linkProvider.getLinkTransactionHistory("THIS_MONTH", formattedFdate, formattedTdate, subAgentID!);
    await cashTransProvider.getCashTranscationHistory("THIS_MONTH", formattedFdate, formattedTdate, "COLLECTION_CASH",subAgentID!,corpCode);
    if(cashTransProvider.qrTranscationHistoryModel != null){
      int cashAmtLen = 0;
      while (cashAmtLen < cashTransProvider.qrTranscationHistoryModel!.data!.length){
        setState(() {
                          ...
                          Card(
                            ...
                            child: Padding(
                              ...
                              child: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  const Icon(Icons.attach_money, color: home1, size: 26),
                                  ...
                                  Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        "₹ ${(
                                            ((cashTranProvider.qrTranscationHistoryModel?.data?.fold<num>(0, (sum, item) => sum + (item.orderAmount ?? 0)) ?? 0) +
                                                (qrProvider.qrTranscationHistoryModel?.data?.fold<num>(0, (sum, item) => sum + (item.orderAmount ?? 0)) ?? 0))
                                        ).toInt()}",
                                        style: const TextStyle(
                                          fontSize: 15,
                                          fontWeight: FontWeight.bold,
