...
import 'package:collection_qr_flutter/core/colors.dart';
import 'package:collection_qr_flutter/data/provider/group/bank_detail_provider.dart';
import 'package:collection_qr_flutter/data/provider/group/bank_account_update_provider.dart';
...
class _BankDetailsScreenState extends State<BankDetailsScreen> {
  ...
  void loadSharedData() async {
    try {
      ...
      // Automatically fetch bank details if customer ID is available
      if (_custId != null) {
  ...
  Future<void> _fetchBankDetails() async {
    ...
    try {
      ...
      await bankAccountUpdateProvider.getBankAccountDetails(int.parse(_custId!));

      if (bankAccountUpdateProvider.bankAccountUpdateResponse != null &&
          bankAccountUpdateProvider.bankAccountUpdateResponse!.data.isNotEmpty) {

        final bankData = bankAccountUpdateProvider.bankAccountUpdateResponse!.data[0];

        setState(() {
          _hasExistingDetails = true;
          _panNumberController.text = bankData.accountHolderName ?? '';
          _accountNumberController.text = bankData.accountNumber ?? '';
          _confirmAccountNumberController.text = bankData.accountNumber ?? '';
      ...
      setState(() {
        _errorMessage = "Failed to fetch bank details";
      });
    } finally {
      setState(() {
        _isLoading = false;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              bankDetailSubmitProvider.bankAccountModel?.message ??
                  (_hasExistingDetails ? "Bank details updated successfully" : "Bank details saved successfully"),
            ),
            backgroundColor: home2,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
        ...
        // Update state to reflect that we now have existing details
        setState(() {
          _hasExistingDetails = true;
        });
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ? const Center(child: CircularProgressIndicator(color: home2))
          : _errorMessage != null
          ? Center(
        child: Column(
